...
### 2026-01-29 v2.6.1 🎉 反応速度改善 + iPhone対応 + 印刷修正！
- ✅ **反応速度を大幅改善**
  - Canvas初期化を初回のみ実行（高速化）
  - 不要なリサイズ処理をスキップ
  - transformSet フラグで重複処理を防止
- ✅ **iPhone対応：練習モードの枠サイズ拡大**
  - 最小サイズを150px→180pxに設定
  - 小さい画面でも書きやすく
- ✅ **印刷時のレイアウト修正**
  - 練習モード・テストモード両方を印刷対応
  - ページまたぎ防止（break-inside: avoid）
  - コントロールボタンを非表示
  - 適切な余白設定（10mm）
